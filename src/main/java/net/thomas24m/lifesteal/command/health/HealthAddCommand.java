...
public class HealthAddCommand {
    ...
    private static int run(CommandContext<ServerCommandSource> context, ServerPlayerEntity player, int amount) {
        double change = LifeStealUtils.addHealth(player, (amount));
        context.getSource().sendFeedback(() -> Text.literal("Added %d to the max health of ".formatted((int) change)).setStyle(Style.EMPTY.withFormatting(Formatting.WHITE)).append(player.getDisplayName().copy()), true);
        return 1;
    }
}
